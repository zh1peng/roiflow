...
# Agent demo (config-first):
# Reads one job JSON and runs the full pipeline via run_pipeline().
# No step-by-step analysis code is hard-coded here.
#
# Usage:
#   Rscript scripts/demo_two_group_agent.R
#   Rscript scripts/demo_two_group_agent.R inst/examples/example_cfg.json

args <- commandArgs(trailingOnly = TRUE)
cfg_path <- if (length(args) >= 1 && nzchar(args[1])) {
  args[1]
} else {
  file.path("inst", "examples", "example_cfg.json")
}
...
t0 <- Sys.time()
cat("\n=== Two-Group Demo (Agent, Config-First) ===\n")
